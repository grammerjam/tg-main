...
const WatchMoreCard = ({ trendingMedia }) => {
    ...
    return (
        <div className='w-full flex flex-col gap-[4px]'>
            <img className='w-full rounded-[10px]' src={trendingMedia.tpathTrending} />
            <div className='w-full flex flex-col gap-[8px]'>
                <div className='flex gap-[8px] items-baseline'>
                    <p className='text-[20px] font-[600]'>{trendingMedia.title}</p>
                    <p className='text-[12px]'>{"Rated: " + trendingMedia.rating}</p>
                </div>
                <div className='flex gap-[8px]'>
                    {trendingMedia.genre.map((genre, index) => {
                        return (
                            <div key={index} className='bg-ma-blue rounded-[10px] px-[8px] py-[4px]'>
                                <p >{genre}</p>
                            </div>
                        )
                    })}
